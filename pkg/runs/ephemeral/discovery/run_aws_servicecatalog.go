...
func AWSServiceCatalogRunSetup(serviceCatalogProductName string) func() error {
	return func() error {
...
func AWSServiceCatalogRun(env string) *demo.Run {
	r := demo.NewRun("3rd-party integration with AWS Service Catalog")
	serviceCatalogProductName := fmt.Sprintf("primaza-demo-orders-%s", env)
	lambdaName := fmt.Sprintf("pmzdemoord%s", env)
	namespace := fmt.Sprintf("services-%s", env)
	r.Setup(AWSServiceCatalogRunSetup(serviceCatalogProductName))
...
`,
	...
	r.Step(
		...
		demo.S("aws servicecatalog provision-product",
			...
			fmt.Sprintf(`Key=QueueURL,Value=$(kubectl get queues.sqs.services.k8s.aws sqs-orders-%s -o jsonpath='{.status.queueURL}' --context kind-worker --namespace %s )`, env, namespace),
			"--no-cli-pager"))
